...
const AdminHomePage = () => {
  ...
  const { products, productsLoading, error: productsError } = useSelector((state) => state.adminProducts);
  ...
  return (
    <div className='max-w-7xl mx-auto p-6'>
      ...
      {productsLoading || ordersLoading ? (
        ...
        <p className='text-center py-8 text-red-500'>Error fetching products: {productsError}</p>
      ) : ordersError ? (
        <p className='text-center py-8 text-red-500'>Error fetching orders: {ordersError}</p>
      ) : (
              ...
              {orders?.length > 0 ? (
                orders.map((order) => (
                  <tr key={order._id} className='border-b hover:bg-gray-50 cursor-pointer'>
                    ...
                    <td className="p-4">₹{order.totalPrice?.toFixed(2) || 0}</td>
